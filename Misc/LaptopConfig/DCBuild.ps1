...
Install-AdcsCertificationAuthority -CACommonName "ADLANG CA" -CAType EnterpriseRootCa -HashAlgorithmName SHA256 -KeyLength 4096 -ValidityPeriod Years -ValidityPeriodUnits 10 -Force
...
c:
cd\
cd .\infrastructure\certificates\internal\wildcard
...
# Create OU Structure
New-ADOrganizationalUnit -Name ADLANG -passthru
New-ADOrganizationalUnit -Name "infrastructure" -Path "ou=ADLANG,dc=ADLANG,dc=local" -PassThru
New-ADOrganizationalUnit -Name "users" -Path "ou=ADLANG,dc=ADLANG,dc=local" -PassThru
New-ADOrganizationalUnit -Name "groups" -Path "ou=ADLANG,dc=ADLANG,dc=local" -PassThru
New-ADOrganizationalUnit -Name "workers" -Path "ou=ADLANG,dc=ADLANG,dc=local" -PassThru
...
# Add CName for WSUS
Add-DnsServerResourceRecordCName -Name "wsus" -HostNameAlias "adlang.local" -ZoneName "adlang.local"
...
Set-GPRegistryValue -Name "adlang_wsus" -key "HKLM\Software\Policies\Microsoft\Windows\WindowsUpdate\AU" -ValueName ScheduledInstallDay -Type Dword -value 1
